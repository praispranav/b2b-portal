...
export class FormProductInformationComponent implements OnInit {
  ...
  buildProductInformationForm() {
    ...
    const sessionValues = this.storageService.getFormValuesFromSession(
      ProviderStorageService.productConstants.productInformation
    );
    if (sessionValues) this.productInformationForm.patchValue(sessionValues);
  }
  get otherDetailsArr() {
    return this.f["otherDetailInfo"] as FormArray;
  }
  ...
  async subProductInformationForm() {
    ...
    formData.otherDetailInfo = formData.otherDetailInfo.map(
      (i) => i.otherDetails
    );
